...
_PROMPT = build_system_prompt("problem_user")


@agent.on_message(AgentTask)
async def handle(ctx: Context, sender: str, task: AgentTask) -> None:
    ctx.logger.info("Problem agent received: %s", task.query[:60])
    confidence = 0
    try:
        ...
        result = (
            f"**🔍 Problem Validator** (Confidence: {confidence}/100)\n\n"
            f"{bullets}\n\n"
            f"**Key Risk:** {data.get('key_risk', 'N/A')}"
        )
    except Exception as exc:
        result = f"**🔍 Problem Validator**: Analysis failed — {exc}"
